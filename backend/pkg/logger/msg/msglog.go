...
type csvLogger struct {
	...
	writer  *csv.Writer
	mu      sync.Mutex
	records chan *MsgRecord
	ticker  *time.Ticker
	done    chan bool
}

var dataLogger = &csvLogger{
	records: make(chan *MsgRecord),
	ticker:  time.NewTicker(5 * time.Second),
	done:    make(chan bool),
}

func initializeCSVLogger(logger *csvLogger, record *MsgRecord) error {
	logger.mu.Lock()
	defer logger.mu.Unlock()

	filename := fmt.Sprintf("msg_%s.csv", record.Timestamp)
...
func MsgLog(record interface{}) error {
	r, ok := record.(*MsgRecord)
	if !ok {
		return errors.New("record type assertion to *MsgRecord failed")
	}
...
func MsgRequest(request interface{}) (interface{}, error) {
	r, ok := request.(*MsgReq)
	if !ok {
