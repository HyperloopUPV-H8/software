...
export const Batteries = () => {
    ...
    return (
        ...
        <Window2 title="High Voltage">
            <GaugeTag name={'Current'} id={'hv'} units={CurrentHV.units} getUpdate={CurrentHV.getUpdate} strokeWidth={110} min={CurrentHV.warningRange[0] ?? 0} max={CurrentHV.warningRange[1] ?? 100} />
            ...
            <GaugeTag name={'DC Bus'} id={'hv-bus'} units={BusHV.units} getUpdate={BusHV.getUpdate} strokeWidth={110} min={BusHV.warningRange[0] ?? 0} max={BusHV.warningRange[1] ?? 100} />
        </Window2>

        <Window2 title="Low Voltage">
            <GaugeTag name={'Current'} id={'lv'} units={CurrentLV.units} getUpdate={CurrentLV.getUpdate} strokeWidth={110} min={CurrentLV.warningRange[0] ?? 0} max={CurrentLV.warningRange[1] ?? 100} />
        ...
        <Window2 title="BOOSTER">
            <GaugeTag name={'Current'} id={'cabinet'} units={CurrentCabinet.units} getUpdate={CurrentCabinet.getUpdate} strokeWidth={110} min={CurrentCabinet.warningRange[0] ?? 0} max={CurrentCabinet.warningRange[1] ?? 100} />
            ...
            <GaugeTag name={'DC Bus'} id={'cabinet-bus'} units={BusCabinet.units} getUpdate={BusCabinet.getUpdate} strokeWidth={110} min={BusCabinet.warningRange[0] ?? 0} max={BusCabinet.warningRange[1] ?? 100} />
        </Window2>
    </div>
    );
